 ...
 * @param string $content     短信验证码
 */
function send_sms($phoneNumer='13790713439', $content='1290', $is_reg=true) {
    vendor('alidayu.vendor.autoload');
    vendor('alidayu.lib.Core.Profile.DefaultProfile');
    \Aliyun\Core\Config::load();

    $arr = db('config')->where('inc_type','sms')->select();
    $configArr = convert_arr_kv($arr,'name','value');
    
    if ($is_reg) {
        //下单验证码
        $config = ['phone'=>trim($phoneNumer),'key'=>$configArr['sms_appkey'],'pwd'=>$configArr['sms_secretKey'],'tempid'=>$configArr['sms_templateCode'],'sign' => $configArr['sms_product'],'value'=>'{"code":"'.$content.'"}'];
    } else {
        //支付成功通知
        $config = ['phone'=>trim($phoneNumer),'key'=>$configArr['sms_appkey'],'pwd'=>$configArr['sms_secretKey'],'tempid'=>'SMS_139233212','value'=>'{"name":"'.$content['name'].'","code":"'.$content['sn'].'"}'];
    }

     //此处需要替换成自己的AK信息
    $accessKeyId = $config['key'];
    $accessKeySecret = $config['pwd'];
    //短信API产品名
    $product = "Dysmsapi";
    //短信API产品域名
    ...
    //必填-短信模板Code
    $request->setTemplateCode($config['tempid']);
    //选填-假如模板中存在变量需要替换则为必填(JSON格式)
    $request->setTemplateParam($config['value']);
    //选填-发送短信流水号
    $request->setOutId(date('ymdhis'));
